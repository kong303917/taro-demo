...
const token = (state = initToken, action: Action<TOKENTYPE, Reducers['token']>) => {
...
const currentAddress = (state = initCurrentAddress, action: Action<CURRENTADDRESSTYPE, Reducers['currentAddress']>) => {
  const { type, payload } = action
  switch (type) {
    case CURRENTADDRESS:
...
const userAddressList = (state = initAddressList, action: Action<USERADDRESSLISTTYPE, Reducers['userAddressList']>) => {
  const { type, payload } = action
  switch (type) {
...
const userAddress = (state = initUserAddress, action: Action<USERADDRESSTYPE, Reducers['userAddress']>) => {
  const { type, payload } = action
  switch (type) {
    case USERADDRESS:
      return payload
